        ...
        for (let i in json) {
            arr.push(
                (() => {
                    let obj = {};
                    for (let x in json[i]) {
                        obj[x] = ifArrOrObjForEval(json[i][x]) ? eval(json[i][x]) : json[i][x]
                    }
...
const getSheet = async (sheetId) => {
    let res = await fetch(`https://docs.google.com/spreadsheets/d/${sheetId}/export?format=csv`)
    const csv = await res.text()
    const json = csv2json(csv)
    return googleSheetsToObj(json)
}
